...
export default async function ParentWalletPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  const { data: children } = await supabase.from("child_profiles").select("*").eq("parent_user_id", user.id).eq("is_active", true);
  ...
  return (
    <ParentShell>
      <div className="grid gap-5">
        ...
        {children?.length ? <WalletAdjustmentForm childrenList={children} /> : <EmptyState title="子どもプロフィールがありません" body="先にプロフィールを作成してください。" href="/setup/child" action="作成する" />}
