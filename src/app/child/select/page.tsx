  ...
  return (
    <AppShell>
      <div className="grid gap-5">
        ...
        {children?.length ? (
          children.map((child) => (
            <Link key={child.id} href={`/child/${child.id}/pin`} className="soft-card grid grid-cols-[78px_1fr_auto] items-center gap-4 rounded-3xl p-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-500">PINを入れて開きます</p>
              </div>
        ...
        <Link href="/parent/dashboard" className="text-center text-sm font-bold text-orange-600">親画面にもどる</Link>
      </div>
    </AppShell>
  );
}
