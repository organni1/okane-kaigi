  ...
  return (
    <div className="grid gap-5">
      <section className="soft-card grid justify-items-center gap-4 rounded-[2rem] p-6 text-center">
        <p className="text-xl font-black text-orange-500">{isApproved ? "うれしいお返事!" : "親からのお返事"}</p>
      ...
      <section className="soft-card rounded-3xl p-5">
        <p className="mb-2 font-black">親からのコメント</p>
        <p className="text-lg">{consultation?.parent_comment || "まだコメントはありません"}</p>
      </section>
      <Button href={`/child/${childId}/home`} variant="outline" className="w-full">ホームにもどる</Button>
    </div>
  );
}
