...
export default async function ParentConsultationsPage({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 text-sm font-bold">
          {filters.map((item) => (
            <Link
              ...
              className={cn(
                "whitespace-nowrap rounded-full px-4 py-2 shadow-sm",
                filter === item.value ? "bg-orange-500 text-white" : "bg-white text-gray-700",
              )}
            >
              {item.label}
            </Link>
